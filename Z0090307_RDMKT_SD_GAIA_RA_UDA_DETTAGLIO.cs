...
{
    [ProcessingLavorazioneAttribute("Z0090307_RDMKT_SD_GAIA_RA_UDA_DETTAGLIO")]
    public class Z0090307_RDMKT_SD_GAIA_RA_UDA_DETTAGLIO : Lavorazione
    {
        private readonly Logger logger;
        private readonly ILavorazioniConfigManager _lavorazioniConfigManager;
        private readonly NormalizzaOperatore _normalizzaOperatore;

        public Z0090307_RDMKT_SD_GAIA_RA_UDA_DETTAGLIO(ILavorazioniConfigManager lavorazioniConfigManager, NormalizzaOperatore normalizzaOperatore)
        {
        ...
        {
            this.TableData = new DataTable("Z0090307_RDMKT_SD_GAIA_RA_UDA_DETTAGLIO");
            ...
            try
            {
                ...
                if (IDFaseLavorazione == 5)
                {
                    ...
                    string query = @"SELECT 
                                        ...
                                        COUNT(*) AS Documenti,
                                        SUM(ISNULL(CAST(NUM_PAG AS int), 0)) / 2 AS fogli, 
                                        SUM(ISNULL(CAST(NUM_PAG AS int), 0)) AS pagine
                                    FROM [RHM_POSTEL].[dbo].[Z0090307_RDMKT_SD_GAIA_RA_UDA_DETTAGLIO]

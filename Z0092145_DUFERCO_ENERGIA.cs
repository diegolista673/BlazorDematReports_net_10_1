...
namespace LibraryLavorazioni
{
    ...
    [ProcessingLavorazioneAttribute("Z0092145_DUFERCO_ENERGIA")]
    public class Z0092145_DUFERCO_ENERGIA : Lavorazione
    {
        private readonly Logger logger;
        private readonly ILavorazioniConfigManager _lavorazioniConfigManager;
        private readonly NormalizzaOperatore _normalizzaOperatore;

        public Z0092145_DUFERCO_ENERGIA (ILavorazioniConfigManager lavorazioniConfigManager, NormalizzaOperatore normalizzaOperatore)
        {
        ...
        {
            this.TableData = new DataTable("Z0092145_DUFERCO_ENERGIA");
            ...
            try
            {
                ...
                if (IDFaseLavorazione == 4)
                {
                    string query = @"SELECT OP_SCAN as operatore,convert(date, DATA_SCAN) as DataLavorazione,COUNT(*) as Documenti,sum(ISNULL(CAST(NUM_PAG AS int),0))/2 as fogli, sum(ISNULL(CAST(NUM_PAG AS int),0)) as pagine
                                    from Z0092145_RDMKT_DUFERCO_FASC_UDA   
                                    where convert(date, DATA_SCAN) >= @startDataScan and convert(date, DATA_SCAN) <= @endDataScan and CAMPO_4 = 'GENOVA' AND OP_SCAN IS NOT NULL
                ...
                if (IDFaseLavorazione == 5)
                {
                    string query = @"SELECT OP_INDEX as operatore,convert(date, DATA_INDEX) as DataLavorazione,COUNT(*) as Documenti,sum(ISNULL(CAST(NUM_PAG AS int),0))/2 as fogli, sum(ISNULL(CAST(NUM_PAG AS int),0)) as pagine
                                    from Z0092145_RDMKT_DUFERCO_FASC_UDA    
                                    where convert(date, DATA_INDEX) >= @startDataDE and convert(date, DATA_INDEX) <= @endDataDE and CAMPO_4 = 'GENOVA' AND OP_INDEX IS NOT NULL

...
namespace LibraryLavorazioni
{
    ...
    [ProcessingLavorazioneAttribute("Z0092959_RDMKT_COM_SEQUALS_PE_UDA")]
    public class Z0092959_RDMKT_COM_SEQUALS_PE_UDA : Lavorazione
    {
        private readonly Logger logger;
        private readonly ILavorazioniConfigManager _lavorazioniConfigManager;
        private readonly NormalizzaOperatore _normalizzaOperatore;

        public Z0092959_RDMKT_COM_SEQUALS_PE_UDA(ILavorazioniConfigManager lavorazioniConfigManager, NormalizzaOperatore normalizzaOperatore)
        {
        ...
        {
            this.TableData = new DataTable("Z0092959_RDMKT_COM_SEQUALS_PE_UDA");
            ...
            try
            {
                ...
                if (IDFaseLavorazione == 4)
                {
                    string query = @"select OP_SCAN as operatore,convert(date, DATA_SCAN) as DataLavorazione, COUNT(*) as Documenti,sum(ISNULL(CAST(NUM_PAG AS int),0))/2 as fogli, sum(ISNULL(CAST(NUM_PAG AS int),0)) as pagine
                                     from Z0092959_RDMKT_COM_SEQUALS_PE_UDA 
                                     where convert(date, DATA_SCAN) >= @startDataScan and convert(date, DATA_SCAN) <= @endDataScan AND CAMPO_U1 = 'DOCUMENTO'
                ...
                //Scansione Tavole
                if (IDFaseLavorazione == 60)
                {
                    string query = @"select OP_SCAN as operatore,convert(date, DATA_SCAN) as DataLavorazione, COUNT(*) as Documenti,COUNT(*) as fogli, sum(ISNULL(CAST(NUM_PAG AS int),0)) as pagine
                                     from Z0092959_RDMKT_COM_SEQUALS_PE_UDA 
                                     where convert(date, DATA_SCAN) >= @startDataScan and convert(date, DATA_SCAN) <= @endDataScan AND CAMPO_U1 = 'tavola'
                                     group by OP_SCAN,convert(date, DATA_SCAN)";

...
namespace LibraryLavorazioni
{
    ...
    [ProcessingLavorazioneAttribute("Z0092154_RDMKT_ESTINTI_UDA")]
    public class Z0092154_RDMKT_ESTINTI_UDA : Lavorazione
    {
        private readonly Logger logger;
        private readonly ILavorazioniConfigManager _lavorazioniConfigManager;
        private readonly NormalizzaOperatore _normalizzaOperatore;

        public Z0092154_RDMKT_ESTINTI_UDA(ILavorazioniConfigManager lavorazioniConfigManager, NormalizzaOperatore normalizzaOperatore)
        {
        ...
        {
            this.TableData = new DataTable("Z0092154_RDMKT_ESTINTI_UDA");
            ...
            try
            {
                ...
                var startDataAcc = StartDataLavorazione.ToString("yyyyMMdd");
                var endDataAcc = EndDataLavorazione == null ? startDataAcc : EndDataLavorazione.Value.ToString("yyyyMMdd");
                ...
                if (IDFaseLavorazione == 2)
                {
                    string query = @"select OP_NORM as operatore, convert(date, DATA_ACCETTAZIONE) as DataLavorazione, COUNT(*) as Documenti,0 as Fogli, 0 as Pagine
                                     from [RHM_POSTEL].[dbo].[Z0092154_RDMKT_ESTINTI_UDA]
                                     where convert(date, DATA_ACCETTAZIONE) >= @startDataAcc and convert(date, DATA_ACCETTAZIONE) <= @endDataAcc
                                     group by OP_NORM, convert(date, DATA_ACCETTAZIONE)
                                     order by convert(date, DATA_ACCETTAZIONE)";
                    ...
                    using (SqlConnection connection = new SqlConnection(_lavorazioniConfigManager.CnxnCaptiva206))
                    {
                        ...
                        command.CommandTimeout = 0;
                        command.Parameters.Clear();
                        command.Parameters.AddWithValue("@startDataAcc", startDataAcc);
                        command.Parameters.AddWithValue("@endDataAcc", endDataAcc);
                        adapter.SelectCommand = command;
                        adapter.Fill(table);
                    }
                }

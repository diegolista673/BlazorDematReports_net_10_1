...
namespace LibraryLavorazioni
{
    ...
    [ProcessingLavorazioneAttribute("Z0020206_COMUNE_DI_SESSA_AURUNCA")]
    public class Z0020206_COMUNE_DI_SESSA_AURUNCA : Lavorazione
    {
        private readonly Logger logger;
        private readonly ILavorazioniConfigManager _lavorazioniConfigManager;
        private readonly NormalizzaOperatore _normalizzaOperatore;

        public Z0020206_COMUNE_DI_SESSA_AURUNCA (ILavorazioniConfigManager lavorazioniConfigManager, NormalizzaOperatore normalizzaOperatore)
        {
        ...
        {
            this.TableData = new DataTable("Z0020206_COMUNE_DI_SESSA_AURUNCA");
            ...
            try
            {
                ...
                if (IDFaseLavorazione == 4)
                {
                    string query = @"SELECT OP_SCAN as operatore,convert(date, DATA_SCAN) as DataLavorazione,COUNT(*) as Documenti,sum(ISNULL(CAST(NUM_PAG AS int),0))/2 as fogli, sum(ISNULL(CAST(NUM_PAG AS int),0)) as pagine
                                    from Z0020206_RDMKT_SESSA_AURUNCA_PE_UDA  

...
namespace LibraryLavorazioni
{
    ...
    [ProcessingLavorazioneAttribute("Z0006650_SDA_PDF_RESI")]
    public class Z0006650_SDA_PDF_RESI : Lavorazione
    {
        private readonly Logger logger;
        private readonly ILavorazioniConfigManager _lavorazioniConfigManager;
        private readonly NormalizzaOperatore _normalizzaOperatore;

        public Z0006650_SDA_PDF_RESI(ILavorazioniConfigManager lavorazioniConfigManager, NormalizzaOperatore normalizzaOperatore)
        {
        ...
        {
            this.TableData = new DataTable("Z0006650_SDA_PDF_RESI");
            ...
            try
            {
                ...
                if (IDFaseLavorazione == 4)
                {
                    string query = @"select OP_SCAN as operatore,CONVERT(date, DATA_SCAN) as DataLavorazione, Count(*) as Documenti, SUM(convert(int,isnull(NUM_PAG,0))/2) AS Fogli, SUM(convert(int,isnull(NUM_PAG,0))) AS Pagine
                                     from Z0006650_SDA_PDF_RESI_UDA_DETTAGLIO 
                                     where STATO_DOC = 'Pubblicato' and CONVERT(date, DATA_SCAN ) >= @startDataScan and CONVERT(date, DATA_SCAN ) <= @endDataScan

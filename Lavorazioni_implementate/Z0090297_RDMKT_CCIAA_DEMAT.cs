    ...
    public class Z0090297_RDMKT_CCIAA_DEMAT : Lavorazione
    {
        ...
        private DataTable FillTable()
        {
            ...
            try
            {
                ...
                {
                    string query = @"select OP_SCAN as operatore,CONVERT(date, DATA_SCAN ,104) as DataLavorazione, Count(*) as Documenti, SUM(convert(int,NUM_PAG))/2 AS Fogli, SUM(convert(int,NUM_PAG)) AS Pagine
                                     from Z0090297_RDMKT_CCIAA_DEMAT_UDA_DETTAGLIO 

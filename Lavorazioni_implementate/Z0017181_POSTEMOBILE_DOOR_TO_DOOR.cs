...
{
    [ProcessingLavorazioneAttribute("Z0017181_PM_GECT_D2D")]
    public class Z0017181_POSTEMOBILE_DOOR_TO_DOOR : Lavorazione
    {
        private readonly Logger logger;
        private readonly ILavorazioniConfigManager _lavorazioniConfigManager;
        private readonly NormalizzaOperatore _normalizzaOperatore;

        public Z0017181_POSTEMOBILE_DOOR_TO_DOOR(ILavorazioniConfigManager lavorazioniConfigManager, NormalizzaOperatore normalizzaOperatore)
        {
            ...
            try
            {
                ...
                if (IDFaseLavorazione == 4)
                {
                    ...
                    string query = @"select OP_SCAN as operatore, convert(date, DATA_SCAN) as DataLavorazione, COUNT(*) as Documenti,isnull(SUM(convert(int,num_pag))/2,0) as Fogli, isnull(SUM(convert(int,num_pag)),0) as Pagine
                                    from Z0017181_RDMKT_PM_GECT_D2D_UDA_DETTAGLIO

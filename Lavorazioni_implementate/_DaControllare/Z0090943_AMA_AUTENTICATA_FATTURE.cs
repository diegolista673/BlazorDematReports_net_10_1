...
namespace LibraryLavorazioni
{
    ...
    [ProcessingLavorazioneAttribute("Z0090943_AMA_AUTENTICATA_FATTURE")]
    public class Z0090943_AMA_AUTENTICATA_FATTURE : Lavorazione
    {
        private readonly Logger logger;
        private readonly ILavorazioniConfigManager _lavorazioniConfigManager;
        private readonly NormalizzaOperatore _normalizzaOperatore;

        public Z0090943_AMA_AUTENTICATA_FATTURE(ILavorazioniConfigManager lavorazioniConfigManager, NormalizzaOperatore normalizzaOperatore)
        {
        ...
        {
            this.TableData = new DataTable("Z0090943_AMA_AUTENTICATA_FATTURE");
            ...
            try
            {
                ...
                if (IDFaseLavorazione == 4)
                {
                    string query = @"select OP_SCAN as operatore,CONVERT(date, DATA_SCAN) as DataLavorazione, Count(*) as Documenti, SUM(convert(int,isnull(NUM_PAG,0))/2) AS Fogli, SUM(convert(int,isnull(NUM_PAG,0))) AS Pagine
                                    from Z0090943_RDMKT_AMATARI_AUT_UDA_DETTAGLIO 
                                    where STATO_DOC = 'indicizzato' and CONVERT(date, DATA_SCAN ) >= @startDataScan and CONVERT(date, DATA_SCAN ) <= @endDataScan

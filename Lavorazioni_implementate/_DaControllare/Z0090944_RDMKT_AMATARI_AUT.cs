...
namespace LibraryLavorazioni
{
    ...
    [ProcessingLavorazioneAttribute("Z0090944_RDMKT_AMATARI_AUT")]
    public class Z0090944_RDMKT_AMATARI_AUT : Lavorazione
    {
        private readonly Logger logger;
        private readonly ILavorazioniConfigManager _lavorazioniConfigManager;
        private readonly NormalizzaOperatore _normalizzaOperatore;

        public Z0090944_RDMKT_AMATARI_AUT(ILavorazioniConfigManager lavorazioniConfigManager, NormalizzaOperatore normalizzaOperatore)
        {
        ...
        {
            this.TableData = new DataTable("Z0090944_RDMKT_AMATARI_AUT");
            ...
            try
            {
                ...
                if (IDFaseLavorazione == 4)
                {
                    string query = @"select OP_SCAN as operatore,CONVERT(date, DATA_SCAN) as DataLavorazione, Count(*) as Documenti, SUM(convert(int,isnull(NUM_PAG,0))/2) AS Fogli, SUM(convert(int,isnull(NUM_PAG,0))) AS Pagine
                                    from Z0090944_RDMKT_AMATARI_AUT_UDA_DETTAGLIO 

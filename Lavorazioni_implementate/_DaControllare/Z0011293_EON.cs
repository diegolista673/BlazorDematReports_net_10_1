...
namespace LibraryLavorazioni
{
    ...
    [ProcessingLavorazioneAttribute("Z0011293_EON")]
    public class Z0011293_EON : Lavorazione
    {
        ...
        public Z0011293_EON(ILavorazioniConfigManager lavorazioniConfigManager, NormalizzaOperatore normalizzaOperatore)
        {
        ...
        //{
        //    this.TableData = new DataTable("Z0011293_EON");
        ...
        //                                FROM Z0011293_RAC_DATI
        //                                WHERE scatola LIKE '%Z0011293_E%' and CONVERT(date, data_index) >= @startDataDe and CONVERT(date, data_index) <= @endDataDe
        ...
        //                                FROM Z0011293_RAC_DATI
        //                                WHERE scatola LIKE '%Z0011293_I%' and CONVERT(date, data_index) >= @startDataDe and CONVERT(date, data_index) <= @endDataDe
        //                                GROUP By op_index, CONVERT(date, data_index)
        //                            ) as t
            ...
            try
            {
                ...
                if (IDFaseLavorazione == 4)
                {
                    string query = @"select CPT_OPERATORE_SCAN as Operatore,CONVERT(DATE, CPT_DATA_SCAN) as DataLavorazione,SUM(convert(int,isnull(CPT_NUM_DOC,0))) as Documenti, SUM(convert(int,isnull(CPT_NUM_DOC,0))) as Fogli, SUM(convert(int,isnull(CPT_NUM_DOC,0)))*2 as Pagine, SUM(convert(int,isnull(CPT_NUM_SCARTI,0))) as Scarti 
                                    from CPT_BATCH 
                                    where  CONVERT(date, CPT_DATA_SCAN) >= @startDataScan and CONVERT(date, CPT_DATA_SCAN) <= @endDataScan and ( CPT_BATCH_NAME like 'Z0011293_EON_E%' or CPT_BATCH_NAME like 'Z0011293_EON_I%' or CPT_BATCH_NAME like 'Z0011293_EON_M%' )
